...
def fetch_arcgis(config, days_back):
    """V535: body moved to collectors.arcgis.fetch. This back-compat
    shim is preserved so callers using `from collector import
    fetch_arcgis` keep working. The actual implementation lives in
    collectors/arcgis.py; the platform-specific quirks (date format
    auto-retry, pagination, returnGeometry=false safeguard) are now
    collocated with the platform module."""
    from collectors.arcgis import fetch
    return fetch(config, days_back)
