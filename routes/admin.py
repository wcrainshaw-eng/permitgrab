...
def admin_start_collectors():
    ...
    try:
        ...
        if not live_daemon:
            ...
            # V471 PR2: write through to server.* so the server module's
            # binding is updated (a bare assignment would only mutate the
            # blueprint's local namespace).
...
def admin_debug_threads():
    ...
    return jsonify({
        ...
        'threads': threads,
        '_s._collector_started': _s._collector_started,
        '_s._collectors_manually_started': _s._collectors_manually_started,
        '_s._startup_done': _s._startup_done,
